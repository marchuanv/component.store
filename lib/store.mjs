...
export class Store extends GUID {
     ...
     * @param { Schema } dataSchema describes the data schema.
    ...
    */
    constructor(metadata, secureContext, dataSchema) {
        ...
        if (dataSchema === null || dataSchema === undefined || !(dataSchema instanceof Schema)) {
            throw new Error(`The dataSchema argument is null, undefined or not an instance of a ${Schema.name}.`);
        }
        super(metadata);
        if (privateBag.has(this)) {
            ...
            const data = dataSchema.empty();
            const extended = Reflection.getExtendedClasses(targetClass).filter(x => x !== Store && x !== GUID);
            const storeId = new GUID({ Id: super.toString(), dataSchema });
            privateBag.set(storeId, data);
            privateBag.set(this, { secureContext, extended, storeId, dataSchema });
        }
    }
    /**
    ...
    */
    set(value, secureContext) {
        ...
        const { storeId, dataSchema } = privateBag.get(this);
        if (Array.isArray(value)) {
            for (const val of value) {
                if (!dataSchema.validate(val)) {
                    throw new Error(`data does not match data schema.`);
                }
            }
        } else {
            if (!dataSchema.validate(value)) {
                throw new Error(`data does not match data schema.`);
            }
        }
