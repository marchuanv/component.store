...
export class Store extends GUID {
    ...
    */
    validate(value) {
        ...
        if (Array.isArray(value)) {
            for (const val of value) {
                for (const def of schema) {
                    if (val[def.key] === undefined) {
                        undefinedFields.push(def);
                    } else {
                        dataTypeFields.push({ data: value[def.key], type: def.type });
                    }
                }
            }
        } else {
            for (const def of schema) {
                if (value[def.key] === undefined) {
                    undefinedFields.push(def);
                } else {
                    dataTypeFields.push({ data: value[def.key], type: def.type });
                }
        ...
        const allowUndefined = undefinedFields.filter(x => x.type === undefined);
        undefinedFields = undefinedFields.filter(def => !allowUndefined.find(def2 => def2.key === def.key));
        if (undefinedFields.length > 0) {
            throw new Error('properties are not in the schema.');
        }
        for (const { data, type } of dataTypeFields) {
            if (type === undefined && data === undefined) {
                continue;
            } else if (type === null && data === null) {
                continue;
            } else {
                if (Reflection.isClass(type) && data instanceof type) {
                    continue;
                }
                if (Reflection.isPrimitiveType(type) && typeof data === type.name.toLowerCase()) {
                    continue;
                }
                throw new Error('property types do not match schema.');
            }
        }
    }
    /**
