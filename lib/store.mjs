...
export class Store extends GUID {
    ...
    */
    constructor(metadata, secureContext, schema, data) {
        ...
        if (!classMetadata.find(member => member.name === 'get' && member.isMethod)) {
            throw new Error(`The ${directStoreExtendClass.name} class requires a get method.`);
        }
        if (!classMetadata.find(member => member.name === 'set' && member.isMethod)) {
            throw new Error(`The ${directStoreExtendClass.name} class requires a set method.`);
        }
        if (!classMetadata.find(member => member.name === 'extended' && member.isProperty)) {
            throw new Error(`The ${directStoreExtendClass.name} class requires a property called extended.`);
        }
        super(metadata);
